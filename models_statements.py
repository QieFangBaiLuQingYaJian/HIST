...
class StarE_Transformer(StarEEncoder):
    ...
    def forward(self, sub, rel, quals):
        ...
        # sub_emb: (128,200), rel_emb: (128,200), qual_obj_emb: (128,6,200), qual_rel_emb: (128,6,200), all_ent: (47156,200), mask: (128,14)
        # bs*emb_dim , ......, bs*6*emb_dim

        stk_inp = self.concat(sub_emb, rel_emb, qual_rel_emb, qual_obj_emb) #sub_emb, rel_emb, qual_rel_emb, qual_obj_emb 是经过聚合后的最终嵌入，把他们全concat起来
        # 所以我只需要到这一步将 sub_emb，rel_emb输出即可 ***
        if self.positional:
        ...
        x = self.encoder(stk_inp, src_key_padding_mask=mask) # 输入Transformer
        ...
        x = torch.mm(x, all_ent.transpose(1, 0)) #将结果和所有实体相乘，最高得分为预测实体
